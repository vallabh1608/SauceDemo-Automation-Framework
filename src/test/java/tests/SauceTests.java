...
//			Assert.assertEquals(inventoryPage.getPageTitle(), Expected,"Valid Login is failed!");
//		}
//		else {
...
//		}
//		
//		List<String> sortedNames = new ArrayList<String>(actualNames);
...
//		    // 1. Add 3 items
//		    for(int i = 0; i < 3; i++) {
//		        // Step A: Refresh the PageFactory cache so it finds the buttons again
//		        inventoryPage.refreshElements(); 
//		        
//		        List<WebElement> inventoryBtns = inventoryPage.getInventoryBtns();
//		        WebElement inventoryBtn = inventoryBtns.get(i);
//		        
//		        inventoryBtn.click();
//		        
//		        // Step B: Refresh again because clicking changed the button's HTML (it's now a 'Remove' button)
//		        inventoryPage.refreshElements(); 
//		        Assert.assertEquals(inventoryPage.getInventoryBtns().get(i).getText(), "Remove", "Button text did not change for item " + i);
//		    }
//		    
//		    // 2. Capture Names (Do this while elements are fresh)
//		    inventoryPage.refreshElements();
...
//		
//		Assert.assertTrue(driver.getCurrentUrl().contains("inventory.html"),"Did not navigate back to the inventory URL!");
...
//		loginPage.login(creader.getString("username"), creader.getString("password"));
//		inventoryPage.refreshElements();
//	    inventoryPage.getInventoryBtns().get(0).click(); // Add first item
//	    inventoryPage.refreshElements();
//	    inventoryPage.getInventoryBtns().get(1).click(); // Add second item
//	    
...
//		
//		Assert.assertTrue(driver.getCurrentUrl().contains("https://www.saucedemo.com/"), "URL does not match the login screen base destination!");
//		
//		driver.get("https://www.saucedemo.com/inventory.html");
//		
//		String currUrlAfterByPass = driver.getCurrentUrl();
//		
...
//	
//	    pages.ProductDetailsPage detailsPage = new pages.ProductDetailsPage(driver);
//	    String actualDetailName = detailsPage.getProductName();
//	    
//	    Assert.assertEquals(actualDetailName, targetedProductName, 
//	        "Image routing discrepancy: Clicking the image loaded the wrong product detail view!");
//	}
//	
//	@Test
//	public void verifyCrossUserCartIsolation() {
//	    // 1. Log in as standard_user and add 2 items
...
//
//	    // 2. Log out standard_user and hard-clear browser cache memory
//	    inventoryPage.clickOnMenuBtn();
//	    inventoryPage.clickOnLogOut();
...
//
//	    // 3. Log in as problem_user and verify their cart is completely empty (0)
...
//	        "SECURITY LEAK: Data isolation failure! problem_user can see items added by standard_user.");
//
//	    // 4. Add an item as problem_user to taint the session state intentionally
//	    driver.get("https://www.saucedemo.com/inventory.html");
//	    inventoryPage.refreshElements();
//	    inventoryPage.getInventoryBtns().get(0).click();
//	    
//	    // 5. Log out problem_user
//	    inventoryPage.clickOnMenuBtn();
//	    inventoryPage.clickOnLogOut();
//	    
//	    // 6. Hard-clear browser storage again to wipe problem_user's leftovers
//	    driver.manage().deleteAllCookies();
//	    js.executeScript("window.localStorage.clear();");
//	    js.executeScript("window.sessionStorage.clear();");
//	    
//	    // 7. Log back in as standard_user and verify a completely pristine isolated cart (0)
//	    loginPage.login("standard_user", creader.getString("password"));
//	    
//	    inventoryPage.refreshElements();
//	    inventoryPage.goTocartPage();
//	    
//	    Assert.assertEquals(cartPage.getCartItemCount(), 0, 
//	        "Data persistence isolation failure: standard_user's new login state caught data contamination.");
//	}
//}
package tests;
